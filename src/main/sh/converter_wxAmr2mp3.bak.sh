...
while [ $3 ]; do
	...
	ls $1 | while read line; do
		...
		if [ ! -f "$2/$line.pcm" ]; then
			/usr/local/bin/ffmpeg -y -i "$1/$line" "$2/${line%.*}.$3" > /dev/null 2>&1 &
			ffmpeg_pid=$!
		...
		/usr/local/bin/ffmpeg -y -f s16le -ar 24000 -ac 1  -acodec pcm_s16le -i "$2/$line.pcm" "$2/${line%.*}.$3" > /dev/null 2>&1 &
		ffmpeg_pid=$!
		while kill -0 "$ffmpeg_pid"; do sleep 1; done > /dev/null 2>&1
		rm "$2/$line.pcm"
...
echo -e "${1%.*}.$2"
rm -rf "${1%.*}.amr"
chown memory:memory 1_20190628_134656.mp3
exit
